...
export const permutations = (tokens, subperms = [[]]) =>
  isEmpty(tokens)
    ...
    : addIndex(chain)(
        (token, idx) =>
          permutations(remove(idx, 1, tokens), map(append(token), subperms)),
        tokens
      );
...
export const relax = powers => {
  ...
  let indexOfTheIncrementable = findIndex(
    e => e < powers[indexOfTheDecrementable] - 1,
    powers
  );
...
export const relevance = row =>
  ...
  row.reduce(
    (previous, current, index, array) =>
      previous + 2 ** (array.length - index) * current,
    0
  );
...
export const makeStackedMatrixOfGenerators = (dimensions, degree) => {
  ...
  if (degree > 0) {
    stackedMatrix = concat(
      stackedMatrix,
      makeStackedMatrixOfGenerators(dimensions, degree - 1)
    );
  }
...
export const upperCases = range(65, 65 + 26).map(charCode =>
...
const Powers = ({ powers, variablesNotation }) =>
  powers.map((power, powerIndex) =>
    variablesNotation === "traditional" ? (
      <PowerTraditionalNotation
        key={powerIndex}
        power={power}
        powerIndex={powerIndex}
      />
    ) : (
      <PowerPedanticNotation
        key={powerIndex}
        power={power}
        powerIndex={powerIndex}
      />
    ...
    <span className="surly">
      ...
      <span>
        {variablesNotation !== "pedantic"
          ? range(0, dimensions)
              .map(d => variableNames[d])
              .join(",")
          : range(0, dimensions).map((d, i, a) => (
              <span key={i}>
                x
                <sub>
                  {d + 1}
...
const Terms = ({ coefficientsNotation, variablesNotation, stackedMatrix }) => (
  <>
    {stackedMatrix.map((powers, index, array) => (
      <span key={index}>
        <Term
          powers={powers}
          index={index}
